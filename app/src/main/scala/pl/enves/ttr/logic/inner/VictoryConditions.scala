...
import pl.enves.ttr.logic.StandardGame.State
import pl.enves.ttr.logic.{StandardGame, Player}
...
private[inner] object VictoryConditions extends Logging {
  ...
  def check(board: StandardGame.State): Option[WinnerData] =
  ...
  private def checkHorizontal(board: StandardGame.State): Seq[Option[WinnerData]] = for (x <- 0 until 2; y <- 0 until 5)
    yield checkRow(board, x, y)

  private def checkVertical(board: StandardGame.State): Seq[Option[WinnerData]] = for (x <- 0 until 5; y <- 0 until 2)
    yield checkColumn(board, x, y)

  private def checkDiagonal(board: StandardGame.State): Seq[Option[WinnerData]] = for (x <- 0 until 2; y <- 0 until 2)
